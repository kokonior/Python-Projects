...
def qualitative_analysis(year, exports, imports):
    ...
    if result > 0:
        table.add_row(
            [year, sum(exports), sum(imports), sum(exports) - sum(imports), "Profit"]
        )
    else:
        table.add_row(
            [year, sum(exports), sum(imports), sum(exports) - sum(imports), "Loss"]
        )
...
if __name__ == "__main__":
    ...
    table = PrettyTable(
        ["YEAR", "EXPORTS", "IMPORTS", "BALANCE OF TRADE", "PROFIT/LOSS"]
    )
    exports = [
        186384,
        171511,
